...
static const char* ignore_dirent[] = {
        ".", "..", ".dgit", ".git", ".gitignore", ".cache", "compile_commands.json"};
static size_t num_ignored_dirents = sizeof(ignore_dirent) / sizeof(ignore_dirent[0]);
...
int handle_hash_object(char* path, char* file_type, int print_hash) {
        ...
        char header[128];
        int header_len =
                snprintf(header, sizeof(header), "%s %lld%c", file_type, (long long)f_size, '\0');
        ...
        int write_name = snprintf(new_file_name, sizeof(new_file_name), ".dgit/objects/%s",
                                  hashed_file_name);
...
int handle_cat_file(char* oid) {
        ...
        char file_name[128];
        int write_name = snprintf(file_name, sizeof(file_name), ".dgit/objects/%s", oid);
