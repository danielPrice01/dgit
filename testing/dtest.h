...
#define RUN_TESTS(...)                                                                   \
        GET_RUN_TEST(_, ##__VA_ARGS__, BAD_TEST, BAD_TEST, RUN_TESTS_YES_ARGS, BAD_TEST, \
                     RUN_TESTS_NO_ARGS)                                                  \
        (__VA_ARGS__)

#define RUN_TESTS_NO_ARGS() return run_tests(0, NULL);
#define RUN_TESTS_YES_ARGS(argc, argv) return run_tests((const int)argc, (const char**)argv);
#define BAD_TEST(...) RUN_TESTS_requires_0_or_2_arguments

#define MAX_RESULT_LEN (9)
#define PERIOD_PADDING (5)
#define SPACE_PADDING (1)
#define MAX_FINAL_COL (MAX_TEST_NAME_LEN + PERIOD_PADDING + (2 * SPACE_PADDING) + MAX_RESULT_LEN)
...
static inline int32_t string_compare(const void* item, const void* to_compare_to,
                                     size_t max_length);
static inline int32_t gid_compare(const void* gid, const void* test, size_t null);
static inline int32_t find_gid(const char* group_name, size_t group_name_len);
static inline int32_t find_index(uint16_t starting_index, uint8_t is_group, const void* elt,
                                 int (*comparator_fn)(const void* item, const void* to_compare_to,
                                                      size_t max_length));
static inline int8_t swap_tests(uint16_t* left_idx, int32_t* right_idx, uint16_t* tests_found);

static inline void drain_pipe(int fd, char* buf, size_t* buf_len);
static inline void execute_test(TestCase* test);
static inline void get_res_fmt(const Result res, char** result_str, char** ansi_col);
static inline void format_result(char* formatted_result, const size_t final_space,
...
int run_tests(const int argc, const char** argv) {
        ...
        size_t final_col = max_len + PERIOD_PADDING + SPACE_PADDING + MAX_RESULT_LEN + 1;
        ...
        for (uint16_t i = 0; i < num_tests; ++i) {
                ...
                // name ... result
                printf("%.*s\x1b[%s%s\x1b[0m\n", (int)final_space, formatted_result, ansi_col,
                       result_str);
        ...
        printf("\nTests passed: [%u/%u]\n\n", tests_passed, num_tests - silenced_tests);
...
static inline int32_t find_gid(const char* group_name, size_t group_name_len) {
        ...
        for (uint16_t i = 0; i < num_groups; ++i) {
                ...
                if (string_compare(group_name + 1, groups[i].name, group_name_len - 2) == 0 &&
...
static inline int32_t find_index(uint16_t starting_index, uint8_t is_group, const void* elt,
        ...
        for (uint16_t i = starting_index; i < num_tests; ++i) {
                const void* arg = is_group ? (const void*)&tests[i] : (const void*)tests[i].name;
